...
def format_climate_data(filename, source, outfile, darrs):
    """Format AquaCrop input data files"""
    template = DATAFILES[filename].read_text()
    timevar = darrs[0].time

    frequency = _get_frequency(timevar.values)
    frequency_code = _get_frequency_code(frequency)

    data_lines = '\n'.join((f'{value:.4f}' for value in values) for values in darrs)

    template_filled = template.format(
        source=source,
        start=timevar.values[0],
